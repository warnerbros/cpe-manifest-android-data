...
import com.wb.cpedata.network.SecureFlxHttpClient;
...
//		String responseString = new String(responseBody, "windows-1252"); // Microsoft!...0x92 == stylized ' in Windows
	...
	public static String fetchFileLastModifiedDateFromUrl(URL url) throws IOException {
		CPEDataParserLogger.d(F.TAG_API, "HttpHelper.fetchFileLastModifiedDateFromUrl " + url);
...
//		String responseString = new String(responseBody, "windows-1252"); // Microsoft!...0x92 == stylized ' in Windows
	...
	protected static byte[] fetchUrlBytes(URL url, boolean checkCache, boolean shouldCache, boolean useFlixsterUa,
		...
		CPEDataParserLogger.d(F.TAG_API, "HttpHelper.fetchUrlBytes " + (checkCache ? "checkCache " : "")
				+ (shouldCache ? "shouldCache " : "") + url);
		byte[] result;
		if (checkCache) {
			result = CPEDataParser.sCacheManager.get(url.hashCode());
		...
		if (shouldCache) {
			CPEDataParser.sCacheManager.put(url.hashCode(), result);
		}
	...
	private static byte[] fetchUrlBytes(HttpURLConnection connection, boolean useFlixsterUa) throws IOException {
		String userAgent = CPEDataParser.getUserAgent();
...
//				throw new HttpUnauthorizedException(connection.getResponseMessage(), ioe);
	...
	public static String postToUrl(String url, List<NameValuePair> parameters) throws IOException {
		CPEDataParserLogger.d(F.TAG_API, "HttpHelper.postToUrl url:" + url + " postValues:" + parameters);
		...
		try {
			...
			CPEDataParserLogger.d(F.TAG_API, "HttpHelper.postToUrl response code:" + response.getStatusLine().getStatusCode());
...
//				throw new HttpUnauthorizedException(responseBuilder.toString().trim());
	...
	public static String postJSONToUrl(String url, String requestString, boolean shouldAuth, int timeoutInMillis) throws IOException {
		...
		CPEDataParserLogger.d(F.TAG_API, "HttpHelper.postToUrl url:" + url + " postValues:" + requestString);
		InputStream content = null;
		try {
			HttpClient httpClient = isSecureFlxUrl(url) ? new SecureFlxHttpClient()
					: new DefaultHttpClient();
			...
			post.setHeader(HTTP.USER_AGENT, CPEDataParser.getUserAgent());
			...
			/*if (!StringHelper.isEmpty(CPEDataParser.getClientCountryCode())){
				post.setHeader(F.CLIENT_COUNTRY, CPEDataParser.getClientCountryCode());
			}

			post.setHeader(F.CLIENT_LANGUAGE, CPEDataParser.getLocale().getLanguage());*/
			...
			CPEDataParserLogger.d(F.TAG_API, "HttpHelper.postToUrl response code:" + response.getStatusLine().getStatusCode());
			CPEDataParserLogger.d(F.TAG_API, "HttpHelper.postToUrl response body:" + responseBuilder.toString().trim());
	...
	public static String getFromUrl(String url, List <NameValuePair> params, List <NameValuePair> headerValues, boolean checkCache, boolean shouldCache) throws IOException {
		...
		CPEDataParserLogger.d(F.TAG_API, "HttpHelper.getFromUrl url:" + url);
		InputStream content = null;
		...
		try {
			...
			if (checkCache) {
				byte[] cacheResult = CPEDataParser.sCacheManager.get(new URL(url).hashCode());
				try {
			...
			get.setHeader(HTTP.USER_AGENT, CPEDataParser.getUserAgent());
			...
			//CPEDataParserLogger.d(F.TAG_API, "HttpHelper.getFromUrl response code:" + response.getStatusLine().getStatusCode());
			//CPEDataParserLogger.d(F.TAG_API, "HttpHelper.getFromUrl response body:" + responseBuilder.toString().trim());
	...
	//			throw new HttpUnauthorizedException(responseBuilder.toString().trim());
			...
			if (shouldCache) {
				...
				CPEDataParser.sCacheManager.put(new URL(url).hashCode(), result.getBytes());
			}
			return result;
	...
	public static String deleteFromUrl(String url, boolean shouldAuth) throws IOException {
		CPEDataParserLogger.d(F.TAG_API, "HttpHelper.deleteFromUrl url:" + url);
		InputStream content = null;
		try {
			HttpClient httpClient = isSecureFlxUrl(url) ? new SecureFlxHttpClient()
					: new DefaultHttpClient();
			...
			CPEDataParserLogger.d(F.TAG_API, "HttpHelper.deleteFromUrl response code:" + response.getStatusLine().getStatusCode());
	...
	public static String putJSONToUrl(String url, JSONObject json, boolean shouldAuth) throws IOException {
		CPEDataParserLogger.d(F.TAG_API, "HttpHelper.putToUrl url:" + url + " postValues:" + json);
		InputStream content = null;
		try {
			HttpClient httpClient = isSecureFlxUrl(url) ? new SecureFlxHttpClient()
					: new DefaultHttpClient();
			...
			put.setHeader(HTTP.USER_AGENT, CPEDataParser.getUserAgent());
			...
			CPEDataParserLogger.d(F.TAG_API, "HttpHelper.putToUrl response code:" + response.getStatusLine().getStatusCode());
			CPEDataParserLogger.d(F.TAG_API, "HttpHelper.putToUrl response body:" + responseBuilder.toString().trim());
	...
	private static String getAuthHeader() {
		return "";//CPEDataParser.getAuthToken() + ":" + CPEDataParser.getDeviceID();
	}
