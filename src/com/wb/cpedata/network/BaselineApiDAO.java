...
public class BaselineApiDAO {
    ...
    public static void getCastActorsImages(final List<CastData> castsDatas, final ResultListener<Boolean> l) {        // to get actors images at NGE loading, adding listeners will be called when each actor's images returns
        if (l != null){
        ...
        if (actorsImagesListener.size() <= 1) {
            ...
            Worker.execute(new Callable<Boolean>() {
                ...
                public Boolean call() throws Exception {
                    for (CastData castData : castsDatas) {
                        ...
                        headers.add(new BasicNameValuePair(X_STUDIO_KEY, CPEDataParser.getStudioXAPIKey()));
