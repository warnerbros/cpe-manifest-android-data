...
public class TheTakeApiDAO {
    ...
    public static void init(){
        ...
        if (StringHelper.isEmpty(theTakeAPIKey)) {
            ...
            try {
                ...
                CPEDataParserLogger.e("ss", ex.getLocalizedMessage());
            }
        }
    }
        ...
        Worker.execute(new Callable<List<ShopCategory>>() {
            ...
            public List<ShopCategory> call() throws Exception {
                ...
                try {
                    ...
                    CPEDataParserLogger.d(F.TAG_API, ex.getLocalizedMessage());
    ...
    public static void getFrameProducts(final double frameTime, ResultListener<List<ShopItemInterface>> l){
        Worker.execute(new Callable<List<ShopItemInterface>>() {
            @Override
            public List<ShopItemInterface> call() throws Exception {
                try {
                    List<NameValuePair> params = new ArrayList<NameValuePair>();
                    params.add(new BasicNameValuePair(Keys.Media, theTakeMediaId));
                    ...
                    Type listType = new TypeToken<ArrayList<ShopItemInterface>>() {
                    }.getType();

                    Gson gson = new GsonBuilder().create();

                    List<ShopItemInterface> products = gson.fromJson(result, listType);
    ...
    public static void getCategoryProducts(final int categoryId, ResultListener<List<ShopItemInterface>> l){
        Worker.execute(new Callable<List<ShopItemInterface>>() {
            @Override
            public List<ShopItemInterface> call() throws Exception {
                try {
                    ...
                    List<ShopItemInterface> resultList = gson.fromJson(result, listType);
                    return resultList;
